...
export interface BestValueOpts {
  /** seller -> reputation score 0..100 (default 50: a neutral prior, so a new seller isn't punished). */
  reputation?: Record<string, number>
  /** seller -> wins already made this session; near-ties go to the seller with FEWER, spreading work. */
  awards?: Record<string, number>
  /** weight on price vs reputation in [0,1]; price gets `priceWeight`, reputation `1 - priceWeight`. */
  priceWeight?: number
}

/** Bids whose value differs by less than this (on the 0..1 scale) count as "close" -> load-balanced. */
const TIE_TOLERANCE = 0.05

/**
 * Deterministic best-value pick: blends price and track record, then breaks near-ties to
 * DISTRIBUTE wins rather than letting one seller take every round. Tie order: fewer prior awards
 * (load-balance) -> earlier bid (latency; pool order is arrival order) -> name (stable).
 *
 * Reputation defaults to a neutral 50, so it degrades toward "cheapest" with no history/options —
 * a testable superset of pickCheapest that the buyer uses as its no-LLM fallback.
 */
export function pickBestValue(bids: Bid[], opts: BestValueOpts = {}): Bid | undefined {
  if (bids.length <= 1) return bids[0]
  const priceWeight = opts.priceWeight ?? 0.6
  const repWeight = 1 - priceWeight
  const minPrice = Math.min(...bids.map((b) => b.priceSol))
  const rep = (by: string) => (opts.reputation?.[by] ?? 50) / 100
  const awards = (by: string) => opts.awards?.[by] ?? 0
  // value in [0,1]: price is scored scale-free as min/price (cheapest = 1), reputation as score/100.
  const priceScore = (p: number) => (p > 0 ? minPrice / p : 1)
  const value = (b: Bid) => priceWeight * priceScore(b.priceSol) + repWeight * rep(b.by)
  // Quantise value so bids within TIE_TOLERANCE share a bucket; ties then go to fairness, then latency.
  const bucket = (b: Bid) => Math.round(value(b) / TIE_TOLERANCE)
  const arrival = new Map(bids.map((b, i) => [b, i]))
  return [...bids].sort((a, b) =>
    bucket(b) - bucket(a) ||                 // higher value bucket first
    awards(a.by) - awards(b.by) ||           // fewer prior wins first (distributes work)
    (arrival.get(a)! - arrival.get(b)!) ||   // earlier bid = lower latency, first
    a.by.localeCompare(b.by),                // stable, deterministic
  )[0]
}
