...
import { settledRound, accessguardRound } from '../../tests/fixtures'
...
describe('RoundCard', () => {
  ...
  it('renders an accessguard delivery as the WCAG score jump, not a raw dump', () => {
    render(<RoundCard round={accessguardRound} />)
    const panel = screen.getByTestId('ag-audit')
    expect(screen.queryByTestId('delivered')).toBeNull()
    expect(within(panel).getByTestId('ag-before').textContent).toBe('42')
    expect(within(panel).getByTestId('ag-after').textContent).toBe('100')
    expect(within(panel).getByTestId('ag-fixes').textContent).toContain('html-lang')
  })
})
