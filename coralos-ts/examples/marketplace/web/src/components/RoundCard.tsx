...
import { AccessGuardPanel } from './AccessGuardPanel'
  ...
  return (
    <article className="round" data-testid="round" data-round={round.round}>
      ...
      {round.delivered && (() => {
        ...
        if (service === 'txline-edge') {
          return <WorldCupPanel edge={round.delivered.data as Parameters<typeof WorldCupPanel>[0]['edge']} />
        }
        if (service === 'accessguard') {
          return <AccessGuardPanel audit={round.delivered.data as Parameters<typeof AccessGuardPanel>[0]['audit']} />
        }
