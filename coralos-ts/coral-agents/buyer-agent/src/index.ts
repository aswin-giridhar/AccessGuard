...
import {
  ...
  selectBids, pickBestValue, formatReputation, verb, messageRound,
...
import { fetchReputation, reputationScores } from './reputation.js'
...
interface PickOpts {
  /** Formatted per-seller history for the LLM prompt. */
  repLines?: string
  /** seller -> reputation score, for the deterministic fallback. */
  repScores?: Record<string, number>
  /** seller -> wins already made this session (spreads close calls across competitors). */
  awards?: Record<string, number>
}

/**
 * Best-value selection via LLM; deterministic best-value fallback (price × reputation, load-balanced).
 * The awards-so-far are shown to the LLM too, so on a close call it is nudged to spread work rather
 * than pile every round on one incumbent. Returns the winner + its reasoning.
 */
async function pickWinner(pool: Bid[], opts: PickOpts = {}): Promise<{ winner: Bid; reason?: string }> {
  if (pool.length === 1) return { winner: pool[0] }
  try {
    const system =
      'You are a buyer choosing the best-value bid for a Solana service. Weigh price against ' +
      "each seller's track record when one is given - a cheap seller that fails verification or " +
      'no-shows is not a bargain. When two bids are close, prefer a seller that has won fewer rounds ' +
      'this session so work is distributed. Reply ONLY with JSON {"by": "<seller name>", "reason": "<short>"}.'
    const awardsLine = opts.awards && Object.keys(opts.awards).length
      ? `\nwins so far this session: ${Object.entries(opts.awards).map(([s, n]) => `${s}=${n}`).join(', ')}`
      : ''
    const user =
  ...
  const winner = pickBestValue(pool, { reputation: opts.repScores, awards: opts.awards })!
  return { winner, reason: 'best value (price x reputation, load-balanced)' }
}
...
await startCoralAgent({ agentName: process.env.AGENT_NAME ?? 'buyer-agent' }, async (ctx) => {
  ...
  const awardsBySeller: Record<string, number> = {}
  ...
  while (true) {
    try {
      ...
      // -- award the best value (price × track record, load-balanced) ----------
      const reps = REPUTATION_URL ? await fetchReputation(REPUTATION_URL) : undefined
      const repLines = reps ? formatReputation(reps) : undefined
      const { winner, reason } = await pickWinner(pool, { repLines, repScores: reputationScores(reps), awards: awardsBySeller })
      await ctx.send(formatAward(round, winner.by, reason), thread, [winner.by])
      awardsBySeller[winner.by] = (awardsBySeller[winner.by] ?? 0) + 1
